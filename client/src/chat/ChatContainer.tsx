...
const ChatContainer: React.FC = () => {
  ...
  ); // Default Width of Chat Container (Desktop)
  const [containerHeight, setContainerHeight] = useState(
    window.innerHeight * 0.5
  ); // Default Height of Chat Container (Mobile)
  ...
  const [displayedQueries, setDisplayedQueries] = useState<
    typeof suggestedQueries
  >([]); // For random selection of suggested query buttons
  ...
  const lastMessageFromChatbot =
    messages.length > 0 && messages[messages.length - 1].sender === "chatbot";
  const showSuggestedQueries =
    !typing && lastMessageFromChatbot && delayShowingSuggestions;
    ...
    if (!typing && lastMessageFromChatbot) {
      const timeout = setTimeout(() => {
        ...
        if (firstChatbotMessage) {
          ...
          const shuffled = [...suggestedQueries].sort(
            () => Math.random() - 0.5
          );
  ...
  useEffect(() => {
    ...
    const handleTouchMove = (e: TouchEvent) => {
      ...
      if (isMobile) {
        ...
        setContainerHeight(
          Math.min(Math.max(newHeight, 100), window.innerHeight)
        );
      }
      // Desktop that is Touchscreen
        ...
        const maxWidth =
          window.innerWidth >= 1920
            ...
            : window.innerWidth * 0.5;
        const newWidth = Math.min(
          Math.max(window.innerWidth - e.touches[0].clientX, 300),
          maxWidth
        );
        setContainerWidth(newWidth);
      }
    ...
    return (
      <button
        ...
        className={`fixed right-0 ${
          isMobile
            ? "bottom-0 w-full flex justify-center items-center"
  ...
  return (
    <>
      ...
      <div
        style={
          isMobile ? { height: containerHeight } : { width: containerWidth }
        }
        className={`fixed z-50 ${
          isMobile
            ? "bottom-0 left-0 w-full rounded-t-lg"
      ...
      >
        ...
        {/* Header */}
        <div className="flex px-3 py-5 bg-gradient-to-r from-[#FDB4C6] to-[#62BB46] bg-[#013F2D]"></div>
          ...
          <div className="max-w-[600px] mx-auto space-y-6">
            ...
            {showSuggestedQueries && (
              <div className="flex flex-wrap justify-center mt-4 gap-3">
                ...
                {suggestedActions
                  .filter(
                    (action) => !action.requiresUserToken || hasAccessToken
                  )
                  .map((item, index) => (
                    <SuggestedActionButton
                      ...
                      requiresUserToken={item.requiresUserToken}
                    />
                  ))}
              </div>
            )}
